...
class JdbcBookRepositoryTest {
    ...
    @DisplayName("Выбрасывается исключение если при обновлении не найдена книга")
    @Test
    void shouldBeExceptionIfBookNotFoundTest() {
        var expectedBook = new Book(5L, "BookTitle_10500", dbAuthors.get(2),
                List.of(dbGenres.get(4), dbGenres.get(5)));

        var exception = assertThrows(EntityNotFoundException.class, () -> repositoryJdbc.save(expectedBook));

        assertTrue(exception.getMessage().contains("Book with id 5 not found"));
    }
